...
class eucalyptus(sos.plugintools.PluginBase):
    ...
    def checkenabled(self):
        if self.isInstalled("eucalyptus-core"):
            return True
        return False
	...
	# List installed Eucalyptus packages
        self.collectExtOutput("rpm -qa | grep euca", timeout = 120)
	# List disk usage
	self.collectExtOutput("df -h", timeout = 120)